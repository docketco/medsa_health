...
// ─────────────────────────────────────────────────────────────────────────────
// Finishes the fully-automated "quote immediately" path into a real held
// policy. This used to run through Medsa's own Stripe account as merchant
// of record for the premium - a deliberate pivot away from that: real HK
// insurers bill premiums directly to the patient, never through a broker,
// and every insurer's own payment page says so explicitly. Medsa isn't in
// the business of collecting insurance premiums or holding client money.
// What Medsa actually gets paid for an automated sale is a flat platform
// fee, billed to the INSURER (insurance_companies.platform_fee_hkd, set in
// medsa-admin) - not the patient, and not calculated as a cut of
// commission (referral fees tied to commission are exactly what the HK
// Insurance Authority has been cracking down on for unlicensed parties).
// So this route creates the policy directly, same validation as before
// (mode must be 'auto', verdict must not be 'needs_review', plan must not
// require an agent, premium always recomputed server-side), and records
// the platform fee owed on the policy for Medsa to invoice the insurer -
// no card charge to the patient at all. The patient's actual premium
// payment gets set up directly with the insurer after this, same as if
// they'd bought straight from the insurer's own site.
// ─────────────────────────────────────────────────────────────────────────────
...
export default async function handler(req, res) {
  ...
  const { data: company } = await supabase.from('insurance_companies').select('institution_ref_id, platform_fee_hkd').eq('name', plan.company_name).maybeSingle()
  ...
  const { data: policy, error: insErr } = await supabase.from('agent_policies').insert({
    ...
    platform_fee_hkd: company?.platform_fee_hkd ?? null,
