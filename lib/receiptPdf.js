...
function drawReceiptHeader(doc, { pageWidth, left, right, clinicName, institution, logo, badgeText, receiptNo, dateStr }) {
  const { GREEN } = RECEIPT_COLORS
...
export async function downloadConsultationReceiptPdf({ transaction: t, record, medications, treatmentPlan: plan, institution }) {
  ...
  const { GREEN, GREEN_LIGHT, GRAY, GRAY_LIGHT, INK, BORDER } = RECEIPT_COLORS
  ...
  drawReceiptHeader(doc, {
    pageWidth, left, right, clinicName, institution, logo,
...
const RECEIPT_BRANDING_COLUMNS = 'receipt_logo_url, receipt_clinic_name, receipt_address, receipt_phone, receipt_footer_note'
