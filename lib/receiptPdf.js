...
// Appends an optional payment reference/approval number (typed in by
// staff from a real standalone card/Octopus terminal's own receipt slip -
// no terminal is actually integrated with this app yet) to the payment
// method label. Truncated rather than wrapped so it can never grow the
// fixed-height card it sits in onto a second line and overlap the row
// below it.
function paymentMethodWithRef(paymentMethod, transactionRef) {
  const label = (paymentMethod||'').replace(/_/g,' ')
  if (!transactionRef) return label
  const ref = transactionRef.length > 24 ? transactionRef.slice(0,24)+'…' : transactionRef
...
function drawReceiptHeader(doc, { pageWidth, left, right, clinicName, institution, logo, badgeText, receiptNo, dateStr, brand }) {
  ...
  doc.text(`Receipt ${receiptNo}`, right, 21, {align:'right'})
...
export async function downloadConsultationReceiptPdf({ transaction: t, record, medications, treatmentPlan: plan, institution, insuranceDetails }) {
  ...
  // Real sequential/year-based number (e.g. R2026-00001) when this
  // transaction has one - falls back to the old id-derived placeholder
  // only for transactions recorded before receipt numbering existed.
  const receiptNo = t.receipt_number || `RCPT-${t.id ? String(t.id).slice(0,8).toUpperCase() : new Date(t.created_at).getTime().toString(36).toUpperCase()}`
  ...
  fieldPair('Payment method', paymentMethodWithRef(t.payment_method, t.transaction_ref), colGap, cy)
  cy += 13
  ...
  doc.setFontSize(9.5); doc.setFont(undefined,'normal'); doc.setTextColor(...INK)
  doc.text('Total charged', totalsX, y); doc.text(`HK$${(consultFee||0).toFixed(2)}`, right, y, {align:'right'}); y += 6
  // A pending-review claim collected the full amount from the patient
  // up front (see handleCollectPendingReviewFull) - this receipt is
  // their own proof of payment to submit to the insurer for
  // reimbursement, not a normal "insurer already paid their share"
  // receipt, so it gets its own clear instructions instead of silently
  // looking identical to a fully self-pay visit with no insurance
  // involved at all.
  if (insuranceDetails?.pendingReview) {
    doc.setFontSize(7.5); doc.setTextColor(...GRAY)
    const bits = [insuranceDetails.planName ? `${insuranceDetails.companyName} (${insuranceDetails.planName})` : insuranceDetails.companyName]
    if (insuranceDetails.policyNumber) bits.push(`Policy ${insuranceDetails.policyNumber}`)
    if (insuranceDetails.claimRef) bits.push(`Claim ${insuranceDetails.claimRef}`)
    const lines = doc.splitTextToSize(`Claim pending review with ${bits.join(' · ')}`, totalsW)
    doc.text(lines, totalsX, y); y += 3.5 * lines.length + 1.5
    doc.setFontSize(7); doc.setFont(undefined,'bold')
    // Two genuinely different facts, not one generic hedge - a
    // claims-plug-in insurer already has this claim (see
    // handleCollectPendingReviewFull), an insurer with none never
    // received anything and the patient still has to file it.
    const noteText = insuranceDetails.claimPluginSubmitted
      ? 'You paid the full amount above because this claim has not been approved yet. It has already been submitted to your insurer electronically for reimbursement review - no further action needed from you beyond waiting to hear back. Keep this receipt for your own records.'
      : 'You paid the full amount above because this claim has not been approved yet. Your insurer does not support electronic claim submission, so you will need to submit this claim yourself - keep this receipt and your claim reference as supporting documentation.'
    const noteLines = doc.splitTextToSize(noteText, totalsW)
    doc.text(noteLines, totalsX, y); y += 3.2 * noteLines.length + 3
    doc.setFont(undefined,'normal'); doc.setFontSize(9.5)
  }
  if (t.insurer_covers>0) {
    doc.setTextColor(...INK); doc.text('Covered by insurer', totalsX, y)
    doc.text(`-HK$${(t.insurer_covers||0).toFixed(2)}`, right, y, {align:'right'}); y += 6
    // Which insurer, which of the patient's own policies, and the real
    // claim reference - without this, "covered by insurer" is a bare
    // number nobody could actually audit against their own insurer.
    // Wrapped to the same narrow totals column the label above sits in
    // (not the full page width) so it reads as directly under that line,
    // not a disconnected note off to the left - splitTextToSize wraps
    // to multiple lines itself if a long insurer name would otherwise
    // overflow past the page edge.
    if (insuranceDetails?.companyName) {
      doc.setFontSize(7.5); doc.setTextColor(...GRAY)
      const bits = [insuranceDetails.planName ? `${insuranceDetails.companyName} (${insuranceDetails.planName})` : insuranceDetails.companyName]
      if (insuranceDetails.policyNumber) bits.push(`Policy ${insuranceDetails.policyNumber}`)
      if (insuranceDetails.claimRef) bits.push(`Claim ${insuranceDetails.claimRef}`)
      const lines = doc.splitTextToSize(bits.join(' · '), totalsW)
      doc.text(lines, totalsX, y); y += 3.5 * lines.length + 1.5
      doc.setFontSize(9.5)
    }
  }
...
export async function downloadTreatmentPlanReceiptPdf({ plan, transaction: txn, institution }) {
  ...
  const brand = resolveBrandColors(institution)
  const GREEN = brand.accent, GREEN_LIGHT = brand.accentLight
  const receiptNo = txn?.receipt_number || `PLAN-${plan.id ? String(plan.id).slice(0,8).toUpperCase() : new Date(plan.created_at).getTime().toString(36).toUpperCase()}`
  const footerY = pageHeight - 16
  const clinicName = institution?.receipt_clinic_name || 'Medsa Health'
  ...
  fieldPair('Payment method', paymentMethodWithRef(txn?.payment_method, txn?.transaction_ref), colGap, cy)
  cy += 13
  ...
  y += 20
  // Always states the real per-session price, discount or not - a
  // patient should be able to see exactly what one session works out to
  // from this receipt alone, not just the lump-sum total. The "vs.
  // normal price" comparison only appears when there's a real reference
  // price on file to compare against.
  if (plan.sessions_paid) {
    doc.setFont(undefined,'normal'); doc.setFontSize(8.5); doc.setTextColor(...GRAY)
    const perSessionText = `Works out to HK$${((plan.price_total||0)/plan.sessions_paid).toFixed(2)} per session`
    doc.text(hasSessionValue ? `${perSessionText}, vs. the normal HK$${plan.session_value.toFixed(2)} per session.` : `${perSessionText}.`, left, y)
    y += 10
  }
...
export async function fetchAndDownloadConsultationReceipt(supabase, transaction) {
  ...
  const { data: medications } = record
    ...
    : { data: [] }
  // A patient/clinic reading "Covered by insurer" has no way to tell
  // which insurer, or which of their policies, actually paid it - only
  // a real insurer name and policy number make the line auditable
  // (matches what they'd see on their own insurer's statement). Pulled
  // from the real claim this transaction settled, when there is one.
  let insuranceDetails = null
  // Real gap this closes: transactions.insurer_name/plan_name (written
  // directly at checkout time - see ClinicOpsApp's billing handlers) is
  // the one source that's always right, including a claim that's still
  // pending review and hasn't billed the insurer anything yet
  // (insurer_covers stays 0 on that transaction, which used to mean
  // this whole block never ran, and the receipt showed no insurer at
  // all even though the claim clearly names one). Falls back to the old
  // join-based lookup only for a transaction recorded before these
  // columns existed.
  if (transaction.claim_ref && transaction.insurer_name) {
    const { data: claim } = await supabase.from('insurance_claims')
      .select('patient_id, plan_id, status').eq('claim_ref', transaction.claim_ref).maybeSingle()
    const { data: policyRow } = claim ? await supabase.from('agent_policies')
      .select('policy_number').eq('patient_id', claim.patient_id).eq('plan_id', claim.plan_id).eq('status', 'active')
      .order('start_date', { ascending: false }).limit(1).maybeSingle() : { data: null }
    // A pending-review claim to an insurer with the claims plug-in
    // enabled (insurance_companies.claims_plugin_enabled) has already
    // been pushed to them electronically at checkout (see
    // handleCollectPendingReviewFull) - a genuinely different, stronger
    // fact than an insurer with no plug-in, where nothing was submitted
    // anywhere and the patient still has to file the claim themselves.
    // Re-checked fresh here (not trusted from checkout time) since
    // this receipt can be re-downloaded long after that happened.
    const { data: company } = claim?.status === 'pending_review'
      ? await supabase.from('insurance_companies').select('claims_plugin_enabled').eq('name', transaction.insurer_name).maybeSingle()
      : { data: null }
    insuranceDetails = {
      companyName: transaction.insurer_name, planName: transaction.plan_name || null,
      policyNumber: policyRow?.policy_number || null, claimRef: transaction.claim_ref,
      // A pending-review claim collected the FULL amount from the
      // patient at checkout since nothing was confirmed by the insurer
      // yet - this is really a reimbursement claim from the patient's
      // side, not a normal "insurer already billed" receipt, so it
      // needs its own instructions rather than looking like a settled
      // claim.
      pendingReview: claim?.status === 'pending_review',
      claimPluginSubmitted: !!company?.claims_plugin_enabled,
    }
  } else if (transaction.claim_ref && transaction.insurer_covers > 0) {
    const { data: claim } = await supabase.from('insurance_claims')
      .select('claim_ref, patient_id, plan_id, insurance_plans(company_name, plan_name)').eq('claim_ref', transaction.claim_ref).maybeSingle()
    if (claim) {
      const { data: policyRow } = await supabase.from('agent_policies')
        .select('policy_number').eq('patient_id', claim.patient_id).eq('plan_id', claim.plan_id).eq('status', 'active')
        .order('start_date', { ascending: false }).limit(1).maybeSingle()
      insuranceDetails = {
        companyName: claim.insurance_plans?.company_name || null,
        planName: claim.insurance_plans?.plan_name || null,
        policyNumber: policyRow?.policy_number || null,
        claimRef: claim.claim_ref,
      }
    }
  }
  return downloadConsultationReceiptPdf({ transaction, record, medications, treatmentPlan: plan, institution, insuranceDetails })
}
