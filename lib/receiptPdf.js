...
export async function downloadConsultationReceiptPdf({ transaction: t, record, medications, treatmentPlan: plan, institution, insuranceDetails }) {
  ...
  if (insuranceDetails?.pendingReview) {
    ...
    const noteLines = doc.splitTextToSize('You paid the full amount above because this claim has not been approved yet. Keep this receipt - if approved, submit it along with your claim reference and any documents your insurer requests to claim reimbursement.', totalsW)
...
export async function fetchAndDownloadConsultationReceipt(supabase, transaction) {
  ...
  if (transaction.claim_ref && transaction.insurer_name) {
    ...
    insuranceDetails = {
      ...
      // patient at checkout (see handleCollectPendingReviewFull) since
      // nothing was confirmed by the insurer yet - this is really a
      // reimbursement claim from the patient's side, not a normal
      // "insurer already billed" receipt, so it needs its own
      // instructions rather than looking like a settled claim.
