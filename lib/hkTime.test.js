...
describe('hkParts / hkHHMM', () => {
  ...
  // Real production crash: "d.getTime is not a function". Several
  // callers (e.g. ClinicOpsApp's queue mapping) store a check-in time as
  // `new Date(x).getTime()` - a plain epoch-ms number, not a Date or ISO
  // string - and pass that number straight into hkHHMM/hkParts.
  it('accepts a raw epoch-ms number, not just a Date or ISO string', () => {
    const ms = new Date('2026-09-11T05:00:00.000Z').getTime()
    expect(hkHHMM(ms)).toBe('13:00')
  })
