...
export class MockInsuranceAdapter extends InsuranceAdapter {
  ...
  async adjudicateClaim(req) {
    ...
    // actually enforced before this - annual_limit_hkd sat there
    // unread, saved but doing nothing to any claim regardless of what a
    // practice manager typed into it. Both are enforced together here: a
    // running per-category total (seeded from this year's prior claims,
    // via categoryUsedThisYear) grows as each item in this visit is
    // processed, so a category's annual room can run out partway through
    // a single visit's own item list, not just across separate visits.
    const categoryUsedRunning = { ...(eligibility.categoryUsedThisYear || {}) }
    ...
    for (const item of (req.items || [])) {
      ...
      let insurableForItem = amount
      if (limits.per_visit_limit_hkd != null) insurableForItem = Math.min(insurableForItem, limits.per_visit_limit_hkd)
      if (limits.annual_limit_hkd != null) {
        const remaining = Math.max(0, limits.annual_limit_hkd - (categoryUsedRunning[category] || 0))
        insurableForItem = Math.min(insurableForItem, remaining)
      }
      categoryOverage += Math.max(0, amount - insurableForItem)
      categoryUsedRunning[category] = (categoryUsedRunning[category] || 0) + amount
    }
