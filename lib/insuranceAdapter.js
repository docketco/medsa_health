...
export class MockInsuranceAdapter extends InsuranceAdapter {
  ...
  async adjudicateClaim(req) {
    ...
    const categoryOverage = (req.items || []).reduce((sum, item) => {
      ...
      // directly.
      const limit = resolveInsuranceCategories(item.code)
        .map(cat => categoryLimits[cat]?.per_visit_limit_hkd)
        .find(l => l != null)
    ...
    return { claimId, status: settlesImmediately ? 'SETTLED' : status, fees, deductibleApplied, annualLimitReached, authorizationCode, adjudicatedAt, verificationFlag: effectiveVerificationFlag, policyTermsOverride: eligibility.policyTermsOverride || null }
  }
