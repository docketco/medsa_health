...
export class MockInsuranceAdapter extends InsuranceAdapter {
  ...
  async adjudicateClaim(req) {
    ...
    // REJECTED still applies when nothing is genuinely insurable at all
    // (no point submitting a claim with nothing to reimburse).
    const status = verificationFlag ? 'PENDING_REVIEW'
      : preauthRequired ? 'PENDING_REVIEW'
      : isReimbursementOnly ? (insurerCoveredAmount > 0 ? 'PENDING_REVIEW' : 'REJECTED')
