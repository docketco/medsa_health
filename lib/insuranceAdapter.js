...
export class MockInsuranceAdapter extends InsuranceAdapter {
  ...
  async checkEligibility(req) {
    ...
    let deductibleQuery = supabase.from('insurance_claims').select('amount, category_breakdown').eq('plan_id', plan.id)
    ...
    // Same prior-claims fetch, summed per category instead of as one flat
    // total - feeds the per-category ANNUAL limit (distinct from the
    // per-visit cap, which needs no history at all). Every claim's own
    // category_breakdown is written at adjudication time below; claims
    // from before that column existed just contribute nothing per
    // category, same as an unset category_limits entry always has.
    const categoryUsedThisYear = {}
    for (const c of (priorClaims||[])) {
      for (const [cat, amt] of Object.entries(c.category_breakdown || {})) {
        categoryUsedThisYear[cat] = (categoryUsedThisYear[cat] || 0) + (amt || 0)
      }
    }
