...
// `visitContext` (optional): { totalGrossAmount, medicalRecordId, clinicId }.
// When supplied, a plan that would need human review for THIS SPECIFIC
// visit (a verification flag, a category/plan pre-authorization
// requirement, or a real payout over the review threshold - see
// adjudicateClaim's status logic) is left out of the returned list
// entirely, not just shown with a warning. Real reasoning: a claim
// needing review can't be billed to the insurer directly at checkout -
// nothing about it is confirmed yet - so front desk clicking it used to
// lead into a dead end (collect the full amount, wait on a review this
// app has no workflow to resolve). The patient already has their own
// self-serve claim submission in the patient app's Insurance tab (see
// PatientApp.jsx's handleSubmitUnverifiedClaim) built exactly for this -
// upload the receipt, bundle it, submit it themselves - so a
// review-needed plan simply isn't offered for direct clinic billing at
// all; the patient pays normally and pursues it there instead. Without
// visitContext (a caller that doesn't have visit specifics yet), this
// precheck is skipped and the old category-only list is returned, same
// as before.
export async function findEligiblePlans(patientId, lineItems, visitContext = null) {
  ...
  if (!visitContext || !visitContext.totalGrossAmount) { candidates.filteredForReviewCount = 0; return candidates }

  // Only a plan with some real category overlap can plausibly need
  // review in the first place - a fully notCovered plan already
  // resolves to $0 insurable/REJECTED on its own (see adjudicateClaim's
  // covered_categories enforcement), so there's nothing to precheck.
  const withReviewCheck = await Promise.all(candidates.map(async c => {
    if (c.notCovered) return { ...c, needsReview: false }
    const adapter = getInsuranceAdapter(c.plan.company_name)
    const preview = await adapter.adjudicateClaim({
      patientId, policyNumber: c.plan.id, clinicId: visitContext.clinicId,
      totalGrossAmount: visitContext.totalGrossAmount, items: lineItems.map(i => ({ code: i.category, description: i.description, amount: i.fee * i.qty })),
      medicalRecordId: visitContext.medicalRecordId, dryRun: true,
    })
    return { ...c, needsReview: preview.status === 'PENDING_REVIEW' }
  }))
  const result = withReviewCheck.filter(c => !c.needsReview)
  // Attached directly on the returned array (harmless - every existing
  // .map/.filter/.length caller only ever touches the indexed elements)
  // so the billing screen can tell "no eligible plans because this
  // patient genuinely has none" apart from "the only one(s) this
  // patient holds all need review for this visit" - those need very
  // different messaging (add a plan, vs. bill directly and let the
  // patient submit it themselves).
  result.filteredForReviewCount = withReviewCheck.length - result.length
  return result
}
...
export class MockInsuranceAdapter extends InsuranceAdapter {
  ...
  async adjudicateClaim(req) {
    ...
    const categoryPreauthRequired = (req.items || []).some(item => {
      const category = resolveItemCategory(item.code)
    ...
    const preauthRequired = (eligibility.preauthThresholdHkd != null && req.totalGrossAmount > eligibility.preauthThresholdHkd) || categoryPreauthRequired
    ...
    if (req.dryRun) {
      return {
        ...
        preauthRequired, categoryPreauthRequired, notCoveredOverage,
      }
    }
