...
export class MockInsuranceAdapter extends InsuranceAdapter {
   ...
   * @returns {Promise<{checked: boolean, verified: boolean}>} checked:false
   *   means "skip this, nothing configured" - never treated as a failure.
   */
  async _verifyPolicyAgainstInsurer(companyName, { policyNumber, hkid }) {
    ...
    if (company.verification_mode === 'roster') {
      let query = supabase.from('insurer_policy_roster').select('status').eq('insurance_company_id', company.id)
      query = policyNumber ? query.eq('policy_number', policyNumber) : query.eq('hkid', hkid)
      const { data: rosterRow } = await query.maybeSingle()
      return { checked: true, verified: !!rosterRow && rosterRow.status === 'active' }
    }
  ...
  async checkEligibility(req) {
    ...
    const { checked: insurerChecked, verified: insurerVerified } = await this._verifyPolicyAgainstInsurer(
    ...
    // Real, insurer-configured deductible when the plan has one set (via
    ...
    const annualDeductible = plan.annual_deductible_hkd ?? 500
    ...
    // the insurer set one. Absent (null) means uncapped, same as today.
    if (plan.overall_annual_limit_hkd != null && usedThisYear >= plan.overall_annual_limit_hkd) {
      return { isEligible: false, planName: '', copayRate: 0, deductibleRemaining: 0, verificationError: `This policy's annual limit of HK$${plan.overall_annual_limit_hkd.toLocaleString()} has already been used this year.` }
    }
    const overallLimitRemaining = plan.overall_annual_limit_hkd != null ? Math.max(0, plan.overall_annual_limit_hkd - usedThisYear) : null
    ...
    return {
      ...
      categoryLimits: plan.category_limits || {},
