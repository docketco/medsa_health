 ...
 * @property {'clinic_ops'|'external_clinic'} [sourceType] - defaults to 'clinic_ops'
 */
...
export class MockInsuranceAdapter extends InsuranceAdapter {
  ...
  async adjudicateClaim(req) {
    ...
    // High-value claims, and claims on an unverified/unreferred
    // practitioner, need human review before automatic settlement,
    // regardless of coverage math - checked first, takes priority over the
    // normal approved/partial/rejected outcome below. An unverified claim
    // still goes through - it just never auto-ticks-off.
    ...
    const { error: claimInsertErr } = await supabase.from('insurance_claims').insert({
      ...
      settled_at: settledAt, verification_flag: verificationFlag, referral_id: referralId,
      // Which clinic actually submitted this - defaults to a native
      // ClinicOps clinic (the only source that existed before the TPA
      // portal). An out-of-network clinic submitting through the TPA
      // portal passes sourceType:'external_clinic' + its own clinicId
      // (an external_clinics.id) instead - same adjudication math, same
      // fee, just tagged so reporting/billing can tell them apart.
    ...
    return { claimId, status: settlesImmediately ? 'SETTLED' : status, fees, deductibleApplied, authorizationCode, adjudicatedAt, verificationFlag }
  }
