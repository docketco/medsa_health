...
export async function findEligiblePlans(patientId, lineItems) {
  ...
  return (policies || [])
    ...
    // plan should I bill this to" picker - they'd never be chosen. A
    // roster-resolved plan is always kept regardless (see above - it has
    // no categories to match against at all, including when the visit's
    // own line items have none set either).
    .filter(m => m.plan.created_by === 'roster-fallback' || m.coveredItems.length > 0)
