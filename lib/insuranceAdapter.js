...
export class MockInsuranceAdapter extends InsuranceAdapter {
  ...
  async adjudicateClaim(req) {
    ...
    // actually billed anything under that category.
    const categoryPreauthRequired = !isReimbursementOnly && (req.items || []).some(item => {
      const category = resolveItemCategory(item.code)
    ...
    const preauthRequired = !isReimbursementOnly && ((eligibility.preauthThresholdHkd != null && req.totalGrossAmount > eligibility.preauthThresholdHkd) || categoryPreauthRequired)
