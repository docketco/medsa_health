...
export async function findEligiblePlans(patientId, lineItems) {
  ...
  return (policies || [])
    ...
    .map(p => {
      ...
      // The real card/policy number on file for this specific link - only
      // meaningful to show for a roster/API-resolved plan, since every
      // patient's link to one insurer via that flow shares a single
      // fallback plan row (see _findOrCreateRosterFallbackPlan). Without
      ...
      const policyNumber = plan.created_by === 'roster-fallback' ? (p.policy_number || null) : null
