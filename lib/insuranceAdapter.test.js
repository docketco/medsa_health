...
let mockPriorClaimsInsurerCovered = null // fed back as that same row's insurer_covered_amount - defaults to mockPriorClaimsSum (100% paid ratio) so every existing test's assumption ("gross billed == insurer paid") holds unless a test opts into a different ratio
...
function makeQueryBuilder(table) {
  ...
  const builder = {
    ...
    gte: async () => ({ data: table === 'insurance_claims' ? [{ amount: mockPriorClaimsSum, category_breakdown: mockPriorClaimsCategoryBreakdown, insurer_covered_amount: mockPriorClaimsInsurerCovered ?? mockPriorClaimsSum }] : [] }),
  }
  return builder
}
...
beforeEach(() => {
  ...
  mockPriorClaimsInsurerCovered = null
...
describe('MockInsuranceAdapter.adjudicateClaim - category vocabulary bridge', () => {
  ...
  // Real bug this test would have caught: a plan whose Annual deductible
  // field was left blank silently got a HK$500/year placeholder applied
  // with no explanation anywhere - a practice manager testing a plan with
  // a small per-visit cap saw the insurer pay $0 and had no way to tell
  // that was the undeclared HK$500 default doing it versus a real
  // configuration bug. usingDefaultDeductible only fires when nothing set
  // a deductible at all - an explicit annual_deductible_hkd:0 must stay
  // silent, same as before.
  it('flags usingDefaultDeductible only when the plan has no deductible configured at all', async () => {
    mockMedicalRecord = { doctor_name: 'Dr. Verified', patient_id: 'p1' }
    mockStaffCredential = { verification_status: 'verified', has_epc: true }
    const adapter = new MockInsuranceAdapter()

    mockPlan = { id: 'plan-nodd', plan_name: 'No Deductible Set', company_name: 'Test Insurer Co', status: 'active', copay_rate: 0 }
    const unset = await adapter.adjudicateClaim({
      patientId: 'p1', policyNumber: 'plan-nodd', clinicId: 'clinic_ops', totalGrossAmount: 350,
      items: [{ code: 'Consultation', description: 'General Consultation', amount: 350 }], medicalRecordId: 'mr1',
    })
    expect(unset.usingDefaultDeductible).toMatch(/HK\$500/)

    mockPlan = { id: 'plan-zerodd', plan_name: 'Zero Deductible', company_name: 'Test Insurer Co', status: 'active', copay_rate: 0, annual_deductible_hkd: 0 }
    const explicit = await adapter.adjudicateClaim({
      patientId: 'p1', policyNumber: 'plan-zerodd', clinicId: 'clinic_ops', totalGrossAmount: 350,
      items: [{ code: 'Consultation', description: 'General Consultation', amount: 350 }], medicalRecordId: 'mr1',
    })
    expect(explicit.usingDefaultDeductible).toBeNull()
  })
  ...
  // Real gap this closes: a preauth request now only ever gets created
  // against a patient's actual held policy (see PreauthRequestsScreen),
  // recording its real policy_number - if this claim ALSO resolves a real
  // verified policy number (via agent_policies) and the two don't match,
  // the GOP must not clear the gate, even if the gop_code/patient/plan all
  // otherwise line up. Ties the GOP to the specific policy it was issued
  // for, not just a patient+plan pairing that could cover more than one
  // real policy.
  it('does not clear the preauth gate when the GOP\'s policy number does not match this claim\'s verified policy', async () => {
    mockPlan = {
      id: 'plan-preauth4', plan_name: 'Preauth Plan', company_name: 'Test Insurer Co', status: 'active',
      copay_rate: 0, annual_deductible_hkd: 0, preauth_threshold_hkd: 300,
    }
    mockMedicalRecord = { doctor_name: 'Dr. Verified', patient_id: 'p1' }
    mockStaffCredential = { verification_status: 'verified', has_epc: true }
    mockAgentPolicyRow = { start_date: '2020-01-01', policy_number: 'POL-123' }
    mockPreauthRequest = { id: 'preauth-3', gop_code: 'GOP-MISMATCH', status: 'approved', used_at: null, expires_at: null, policy_number: 'POL-999' }

    const adapter = new MockInsuranceAdapter()
    const result = await adapter.adjudicateClaim({
      patientId: 'p1', policyNumber: 'plan-preauth4', clinicId: 'clinic_ops',
      totalGrossAmount: 500,
      items: [{ code: 'Consultation', description: 'General Consultation', amount: 500 }],
      medicalRecordId: 'mr1', gopCode: 'GOP-MISMATCH',
    })
  ...
  it("counts a category's annual usage from prior claims this year, not just this visit's own items", async () => {
    ...
    mockMedicalRecord = { doctor_name: 'Dr. Verified', patient_id: 'p1' }
    mockStaffCredential = { verification_status: 'verified', has_epc: true }
    mockPriorClaimsSum = 80 // that prior claim's gross bill - matches the category breakdown below at a 100% paid ratio
  ...
  // Real bug this test would have caught, confirmed live: a per-category
  // annual limit counted each prior claim's GROSS bill against the cap,
  // not what the insurer actually paid - a claim that settled for only
  // part of its bill (deductible/copay covering the rest) burned its
  // FULL gross amount from the category's yearly budget, and a claim
  // that came back REJECTED ($0 ever owed) burned its gross amount too.
  // Two low/no-payout prior claims alone could exhaust a real annual cap
  // this way, rejecting a later visit for a "cap already used up" that
  // was never real spend.
  it('counts a category\'s annual usage from what the insurer actually paid on prior claims, not their gross bill', async () => {
    mockPlan = {
      id: 'plan-cat2', plan_name: 'Test Plan', company_name: 'Test Insurer Co', status: 'active',
      copay_rate: 0, annual_deductible_hkd: 0,
      category_limits: { Outpatient: { annual_limit_hkd: 1000 } },
    }
    mockMedicalRecord = { doctor_name: 'Dr. Verified', patient_id: 'p1' }
    mockStaffCredential = { verification_status: 'verified', has_epc: true }
    // A prior claim that billed $700 gross but the insurer only actually
    // paid $180 of it (deductible/copay covered the rest) - the category
    // cap should only be down $180, not the full $700.
    mockPriorClaimsSum = 700
    mockPriorClaimsInsurerCovered = 180
    mockPriorClaimsCategoryBreakdown = { Outpatient: 700 }

    const adapter = new MockInsuranceAdapter()
    const result = await adapter.adjudicateClaim({
      patientId: 'p1', policyNumber: 'plan-cat2', clinicId: 'clinic_ops',
      totalGrossAmount: 700, items: [{ code: 'Consultation', description: 'Visit', amount: 700 }],
      medicalRecordId: 'mr1',
    })

    // $1000 cap - $180 real prior usage = $820 room - this $700 visit
    // fits entirely within it, not rejected for a cap that was never
    // really used up.
    expect(result.fees.insurerCoveredAmount).toBe(700)
    expect(result.fees.patientPayableTotal).toBe(0)
  })

  // Same bug, the sharper case: a prior claim that was fully REJECTED
  // (insurer paid $0, nothing was ever owed) must contribute nothing at
  // all to the category's annual usage - not its gross bill.
  it('a fully rejected prior claim contributes nothing to a category\'s annual usage', async () => {
    mockPlan = {
      id: 'plan-cat3', plan_name: 'Test Plan', company_name: 'Test Insurer Co', status: 'active',
      copay_rate: 0, annual_deductible_hkd: 0,
      category_limits: { Outpatient: { annual_limit_hkd: 1000 } },
    }
    mockMedicalRecord = { doctor_name: 'Dr. Verified', patient_id: 'p1' }
    mockStaffCredential = { verification_status: 'verified', has_epc: true }
    mockPriorClaimsSum = 350
    mockPriorClaimsInsurerCovered = 0
    mockPriorClaimsCategoryBreakdown = { Outpatient: 350 }

    const adapter = new MockInsuranceAdapter()
    const result = await adapter.adjudicateClaim({
      patientId: 'p1', policyNumber: 'plan-cat3', clinicId: 'clinic_ops',
      totalGrossAmount: 1000, items: [{ code: 'Consultation', description: 'Visit', amount: 1000 }],
      medicalRecordId: 'mr1',
    })

    // Full $1000 cap still available - the rejected prior claim's $350
    // gross bill never actually reduced it.
    expect(result.fees.insurerCoveredAmount).toBe(1000)
  })
